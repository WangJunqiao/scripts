...
cmd = %Q(grep -o '/videos\\?[a-zA-Z0-9\\?&%=_:\\.]*' #{file} > video-index.log)
puts "#{cmd}"
`#{cmd}`

puts "\nParsing all video id requests:"
cmd = %Q(grep -o '/videos/[0-9]+\\?[a-zA-Z0-9\\?&%=_:\\.]*' #{file} > video-id.log)
puts "#{cmd}"
`#{cmd}`

puts "\nParsing all show index requests:"
cmd = %Q(grep -o '/shows\\?[a-zA-Z0-9\\?&%=_:\\.]*' #{file} > show-index.log)
puts "#{cmd}"
`#{cmd}`

puts "\nParsing all show id requests:"
cmd = %Q(grep -o '/shows/[0-9]+\\?[a-zA-Z0-9\\?&%=_:\\.]*' #{file} > show-id.log)
puts "#{cmd}"
`#{cmd}`
